...
function payText(job: any) {
  ...
  if (job.compModel === "commission" || job.compModel === "hybrid") {
    parts.push(`Commission: ${job.payMin ?? "-"}%`);
  }
  if (job.compModel === "hourly" || job.compModel === "hybrid") {
    parts.push(`Wage: ${job.payMax ?? job.payMin ?? "-"} ${job.payUnit}`);
  }
  if (job.compModel === "booth_rent") {
    parts.push(`Rent: ${job.payMin ?? "-"} ${job.payUnit}`);
  }
  return parts.join(" · ") || "—";
}
    ...
    <div className="space-y-4">
      ...
      <div className="hidden md:block sticky top-0 z-10 rounded-xl border border-slate-800 bg-zinc-950/80 backdrop-blur p-3">
        <div className="flex items-center gap-3">
          {/* spacer to align with photo column */}
          <div className="w-[208px]" />
          <div className="grid grid-cols-7 gap-3 flex-1 text-sm text-slate-300">
            <div className="flex items-center gap-2"><Scissors className="h-4 w-4" /> Business & Title</div>
            <div className="flex items-center gap-2"><Scissors className="h-4 w-4" /> Service</div>
            <div className="flex items-center gap-2"><Briefcase className="h-4 w-4" /> Schedule</div>
            <div className="flex items-center gap-2"><Award className="h-4 w-4" /> Experience</div>
            <div className="flex items-center gap-2"><BadgeDollarSign className="h-4 w-4" /> Compensation</div>
            <div className="flex items-center gap-2"><BadgeDollarSign className="h-4 w-4" /> Payment/Wage</div>
      ...
      {jobs.length === 0 ? (
        <div className="rounded-xl border border-slate-800 bg-zinc-950/60 p-6 text-slate-200">
          No jobs yet. Try posting one or run <code>npm run seed</code>.
        </div>
      ) : (
              ...
              >
                ...
                <div className="hidden md:flex items-stretch gap-3 p-3">
                  ...
                  <div className="grid grid-cols-7 gap-3 flex-1 items-center">
                    ...
                    {/* Schedule */}
                    <div className="text-sm text-slate-100">{titleize(job.schedule || undefined) || "—"}</div>

                    {/* Experience */}
                    <div className="text-sm text-slate-100">{job.experienceText || "Any"}</div>

                    {/* Compensation */}
                    <div className="text-sm text-slate-100">{titleize(job.compModel)}</div>
                ...
                <div className="md:hidden p-3 space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm pt-2">
                    ...
                    <div><span className="text-slate-400">Schedule: </span>{titleize(job.schedule || undefined) || "—"}</div>
                    <div><span className="text-slate-400">Experience: </span>{job.experienceText || "Any"}</div>
                    <div><span className="text-slate-400">Comp: </span>{titleize(job.compModel)}</div>
